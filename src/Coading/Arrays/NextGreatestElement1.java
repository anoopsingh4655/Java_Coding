...
public class NextGreatestElement1 {
    ...
    public static void main(String[] args){
        ...
        int[] result = nextGreatestElement(nums1, nums2);
    ...
    public static int[] nextGreatestElement(int[] nums1, int[] nums2){
        ...
        for(int i = 0 ; i < nums1.length; i++){
            if(nextMap.containsKey(nums1[i])){
                int nextGreatest = nextMap.get(nums1[i]);
                if(nextGreatest == 0 || nextGreatest < nums1[i]){
                    result[i] = -1;
                }else if(nextGreatest > nums1[i]){
